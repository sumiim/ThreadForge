...
if (Test-Path -LiteralPath $bundlePackages -PathType Container) {
    ...
    & $venvPython -m pip install (Join-Path $repoRoot "pico-legacy-runtime") (Join-Path $repoRoot "agent-orchestrator") (Join-Path $repoRoot "sandbox-workers") (Join-Path $repoRoot "local-worker")
}
