...
已配对的 Worker Companion 会在后台服务启动后立即检查稳定版，不依赖 WebSocket 握手成功，之后默认每 5 分钟检查一次。检查期间若正在执行任务则跳过本轮；网络错误会在 30 秒后续传重试，旧版本及本地数据保持不变。下载接口支持单段 HTTP Range，Worker 保留未完成的 `.download` 文件并从已有字节继续；完成后仍会校验签名清单、文件大小和 SHA-256，再以静默参数启动安装器，主动停止当前进程让安装器完成替换并重新启动服务。

更新状态按“检查、下载、安装、当前版本、失败”写入本机 `update-status.json` 和轮转日志，并通过 Worker WebSocket 上报有界元数据。前端会展示下载百分比和重试状态，不上报本地路径、设备令牌或模型密钥。CLI 与后台服务共享同一套 PyInstaller 运行时，避免在安装包中重复压缩依赖。
