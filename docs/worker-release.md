...
已配对的 Worker Companion 会在握手完成后立即检查稳定版，之后默认每 5 分钟检查一次。检查期间若正在执行任务则跳过本轮；网络错误或清单校验失败会退避 30 分钟，旧版本继续提供服务。发现新版本后，Worker 会先在本地临时目录下载并校验签名清单、文件大小和 SHA-256，再以静默参数启动安装器，主动停止当前进程让安装器完成替换并重新启动服务。更新失败不会删除当前可用版本。
