...
首次进入工作台时，前端会检查当前账号是否有在线且协议兼容的 Companion。已绑定但离线时会先尝试唤醒；没有可用工作区时，用户可在“新建会话”窗口直接看到带真实下载进度的安装入口。下载包由中央服务校验签名清单和制品 SHA-256 后同源转发。已连接但尚未授权目录时，页面只显示目录授权窗口，不重复下载。
